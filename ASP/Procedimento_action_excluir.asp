...
<% 
		if err = 0 and id <> "" then
			...
			For i=0 to ubound(array_id)
				sql_id = sql_id & "Procedimento.cod_procedimento = " & Trim(array_id(i)) & " OR "
														 'campo texto, entao" & Trim(array_id(i)) & " OR "
														 'caso numerico '" & Trim(array_id(i)) & "' OR "
			Next
			sql_id = left(sql_id,(len(sql_id)-4))
			stringSQL = "DELETE * FROM Procedimento WHERE "&sql_id&""
...
%>
			<table border="0" width="100%" height="8" cellpadding="2" align="center">
			  <tr bgcolor="#0099FF"> 
				<td colspan="7" height="1" align="center"> <font size="2" color="FFFFFF"><b><font face="Verdana, Arial, Helvetica, sans-serif">Seus dados foram excluidos com sucesso!</font></b></font> </td>
			  </tr>
			</table>
		<%else%>
