...
Dim objConn, strQuery, sql_query, RsQuery, campo, sql, cod_servico, ObjRs

'Atrubuição dos valores as respectivas variáveis
	dim desc_servico    : desc_servico         = TRIM(request.form("desc_servico")) & ""
	dim valor_Servico   : valor_Servico        = TRIM(request.form("valor_Servico")) & ""
	

'--------------------------------------------------
cod_servico          = trim(Request.Form("cod_servico"))

Set objConn =  Server.CreateObject("ADODB.Connection")
objConn.Open "DBQ=" & Server.MapPath(Caminho_Banco) & ";Driver={Microsoft Access Driver (*.mdb)}","username","password"

                       strQuery = "UPDATE servico "
            strQuery = strQuery & "   SET desc_servico      = '" & desc_servico & "',"
            strQuery = strQuery & "       valor_Servico     = '" & valor_Servico & "'"
            strQuery = strQuery & " WHERE cod_servico       =  " & cod_servico
			...
			<table border="0" width="100%" height="8" cellpadding="2" align="center">
				<tr bgcolor="#0099FF"> 
					<td colspan="7" height="1" align="center"> <font size="2" color="FFFFFF"><b><font face="Verdana, Arial, Helvetica, sans-serif"> 
						O servico <b>"<%=desc_servico%>"</b> foi alterado com sucesso!</font></b></font> 
					</td>
				</tr>
			</table>
			<%else%>
